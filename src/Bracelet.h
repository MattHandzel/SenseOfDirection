...
public:
  ...
  VibrationMotor(int pin, float position, float arcLength)
      : m_pin{pin}, m_position{position}, m_arcLength{arcLength} {
    pinMode(pin, OUTPUT);
  }
...
<<<<<<< HEAD
    Serial.println("setting power of " + String(m_pin) + " to " +
                   String(power));
=======
>>>>>>> 2e5502800fd84ec317faef1137e8266979c0f935
  ...
  void SetArcLength(float arcLength) { this->m_arcLength = arcLength; }

  // Set the position (in case they want to change it later)
  void SetPosition(float position) { this->m_position = position; }
...
<<<<<<< HEAD
    ...
    Serial.println("I am " + String(m_pin) + " and I am setting my power to " +
                   String(power));
    this->SetPower(power);
  }
};
enum BraceletStatuses { CALIBRATING, ERROR, RUNNING }; 
=======
    power = -power * power + 1;
...
class Bracelet {
  ...
  int AVALIABLE_PINS[4] = {A1, A2, A3,
                           A6}; // A6 - East, A1 - N, A3 - West, A2 - South
...
public:
  ...
  void GenerateMotors() {
    for (int i = 0; i < this->m_nMotors; i++) {
      this->m_motors[i] = new VibrationMotor(
          this->AVALIABLE_PINS[i], (TAU / m_nMotors) * i, this->m_kARCLENGTH);
    }
  }
  ...
  void SetStatus(BraceletStatuses s) { status = s; }
